		...
		<div style={{ border: '2px solid #000', padding: '6px 8px', marginTop: '10px' }} className="blogpost">
			...
			{show ?
				<>
					...
					<p className="post_likes">LIKES: {blog.likes} <button className="button-like" onClick={() => handleLike(blog)}>like</button></p>
					<br />
					{own ? <button className="button-delete" onClick={() => handleRemove(blog.id)}>Remove</button> : null}
				</>
				: null}
